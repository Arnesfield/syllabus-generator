<template>
<v-dialog
  v-model="show"
  :persistent="true"
  transition="fade-transition"
  width="800"
  scrollable
>
  <v-card v-if="show">

    <v-progress-linear
      color="warning"
      height="3"
      :active="loading"
      v-if="loading"
      indeterminate
      background-color="primary lighten-1"
      class="ma-0 primary lighten-1"
    />

    <v-toolbar
      dark
      card
      color="primary lighten-1"
    >
      <v-btn
        icon
        dark
        :disabled="loading"
        @click="show = false"
        @keypress.enter="show = false"
      >
        <v-icon>close</v-icon>
      </v-btn>
      <v-toolbar-title
        v-text="mode == 'add' ? 'Add Curriculum' : 'Update Curriculum'"
      />
      <template v-if="mode == 'edit'">
        <v-spacer/>
        <v-tooltip top>
          <v-btn
            icon
            dark
            slot="activator"
            :disabled="loading"
            @click="deleteItem"
            @keypress.enter="deleteItem"
          >
            <v-icon>delete</v-icon>
          </v-btn>
          <span>Delete</span>
        </v-tooltip>
      </template>
    </v-toolbar>

    <v-card-text>
      <v-form ref="form" v-model="form">

        <!-- title -->

        <v-layout align-center>
          <v-flex hidden-xs-only sm4>
            <v-subheader>
              <div>
                <div>Curriculum label</div>
                <div class="caption" v-text="'(e.g. 2015, 2017, etc.)'"/>
              </div>
            </v-subheader>
          </v-flex>
          <v-flex sm8>
            <v-text-field
              label="Curriculum label"
              placeholder="(e.g. 2015, 2017, etc.)"
              color="primary lighten-1"
              v-model="label"
              :disabled="loading"
              prepend-icon="label"
              :rules="[$fRule('required')]"
              required
            />
          </v-flex>
        </v-layout>

        <!-- status -->

        <v-layout align-center>
          <v-flex hidden-xs-only sm4>
            <v-subheader>Status</v-subheader>
          </v-flex>
          <v-flex sm8>
            <select-status
              v-model="status"
              :items="statusTypes"
              :loading="loading"
              required
            />
          </v-flex>
        </v-layout>

        <!-- latest -->

        <v-layout align-center class="mb-1">
          <v-flex hidden-xs-only sm4>
            <v-subheader>
              <div>
                <div>Latest</div>
                <div
                  class="caption"
                >set this as the
                  <strong>latest</strong>
                </div>
              </div>
            </v-subheader>
          </v-flex>
          <v-flex sm8>
            <v-switch
              :label="latest ? 'This is the latest :)' : 'This is not the latest :('"
              v-model="latest"
            />
          </v-flex>
        </v-layout>

        <!-- content -->

        <template v-if="content.length">
          <v-layout
            align-baseline
            :key="i"
            v-for="(item, i) in content"
          >
            <v-tooltip top>
              <v-btn
                icon
                flat
                small
                color="error"
                slot="activator"
                @click="content.splice(i, 1)"
                :disabled="loading || content.length == 1"
              >
                <v-icon small>close</v-icon>
              </v-btn>
              <span>Remove</span>
            </v-tooltip>

            <v-layout
              row
              wrap
              align-baseline
            >
              <v-flex xs12 sm4>
                <v-text-field
                  label="Label"
                  v-model="item.label"
                  :disabled="loading"
                  :rules="[$fRule('required')]"
                  required
                />
              </v-flex>

              <v-flex xs12 sm8>
                <markdown-textarea
                  :md-view="false"
                  v-model="item.text"
                  placeholder="Enter description"
                  t-add-class="my-medium-textarea pa-2"
                  :ref="'mdDescription-' + i"
                  required
                />
              </v-flex>
            </v-layout>

          </v-layout>
        </template>

      </v-form>
    </v-card-text>

    <v-divider/>
    <v-card-actions>
      <v-btn
        flat
        tabindex="0"
        :disabled="loading"
        @click="show = false"
        @keypress.enter="show = false"
        v-text="'Cancel'"
      />

      <v-spacer/>

      <v-tooltip top>
        <v-btn
          icon
          tabindex="0"
          slot="activator"
          :disabled="loading"
          @click="clear"
          @keypress.enter="clear"
        >
          <v-icon color="primary lighten-1">restore</v-icon>
        </v-btn>
        <span>Reset</span>
      </v-tooltip>
      
      <v-tooltip top>
        <v-btn
          dark
          icon
          color="grey lighten-1"
          slot="activator"
          @click="autoLabel"
        >
          <v-icon small>build</v-icon>
        </v-btn>
        <span>Auto label</span>
      </v-tooltip>

      <v-tooltip top>
        <v-btn
          dark
          icon
          color="primary lighten-1"
          slot="activator"
          @click="addCurriculum"
        >
          <v-icon>add</v-icon>
        </v-btn>
        <span>Add curriculum</span>
      </v-tooltip>

      <v-btn
        color="primary lighten-1"
        tabindex="0"
        :disabled="loading || !form || content.length == 0"
        @click="submit"
        @keypress.enter="submit"
        v-text="mode == 'add' ? 'Add' : 'Update'"
      />
    </v-card-actions>

  </v-card>
</v-dialog>
</template>

<script>
import qs from 'qs'
import find from 'lodash/find'
import SelectStatus from '@/include/SelectStatus'
import MarkdownTextarea from '@/include/MarkdownTextarea'

export default {
  name: 'dialog-manage-curriculum',
  components: {
    SelectStatus,
    MarkdownTextarea
  },
  data: () => ({
    url: '/curriculum/manage',
    deleteUrl: '/curriculum/delete',
    show: false,
    loading: false,
    form: false,
    mode: 'add',
    item: null,

    statusTypes: [
      { text: 'Activated', icon: 'check_circle', color: 'success', value: 1 },
      { text: 'Deactivated', icon: 'cancel', color: 'warning', value: 0 }
    ],

    // values
    id: null,
    label: null,
    latest: false,
    status: null,
    content: []
  }),
  watch: {
    show(e) {
      if (!e) {
        this.clear()
      }
    }
  },

  methods: {
    addItem() {
      this.mode = 'add'
      this.item = null
      this.show = true
      this.clear()
    },
    editItem(item) {
      this.mode = 'edit'
      
      this.item = item
      this.setValuesFromItem(item)

      this.show = true
    },
    deleteItem() {
      let item = this.item
      this.$bus.$emit('dialog--global.confirm.show', {
        item: item,
        title: 'Delete curriculum',
        subtitle: item.label,
        msg: 'This will permanently delete the curriculum.',
        btn: {
          text: 'Delete',
          color: 'error'
        },
        fn: (onSuccess, onError, doClose, fn) => {
          this.$http.post(this.deleteUrl, qs.stringify({
            id: item.id
          })).then(res => {
            console.warn(res.data)
            if (!res.data.success) {
              throw new Error('Request failure.')
            }

            this.$bus.$emit('snackbar--show', 'Curriculum deleted.')
            this.$bus.$emit('manage--curriculum.update')
            this.loading = false
            this.show = false
            onSuccess()
          }).catch(e => {
            console.error(e)
            this.$bus.$emit('snackbar--show', {
              text: 'Unable to delete curriculum.',
              btns: {
                text: 'Retry',
                icon: false,
                color: 'accent',
                cb: (sb, e) => {
                  sb.snackbar = false
                  fn(onSuccess, onError, doClose, fn)
                  // this.deleteComment(item)
                }
              }
            })
            onError()
            doClose()
          })
        }
      })
    },

    addCurriculum() {
      this.content.push({
        label: null,
        text: null
      })
    },

    autoLabel() {
      this.content.forEach((e, i) => {
        e.label = String.fromCharCode(97 + i)
      })
    },

    setValuesFromItem(item) {
      // set values
      this.id = item.id
      this.label = item.label
      this.latest = Boolean(Number(item.latest))
      this.status = find(this.statusTypes, { value: Number(item.status) })
      this.content = item.content ? JSON.parse(JSON.stringify(item.content)) : []
    },

    submit() {
      if (!this.$refs.form.validate()) {
        return
      }

      let data = new FormData()

      data.append('mode', this.mode)
      data.append('label', this.label)
      data.append('latest', this.latest)
      data.append('status', this.status.value)

      data.append('content', JSON.stringify(this.content))

      if (this.mode == 'edit') {
        data.append('id', this.id)
      }

      this.loading = true
      this.$http.post(this.url, data).then(res => {
        console.warn(res.data)
        if (!res.data.success) {
          throw new Error('Request failure.')
        }

        let msg = this.mode == 'add'
          ? 'Added curriculum successfully.'
          : 'Updated curriculum successfully.'
        this.$bus.$emit('snackbar--show', msg)
        this.$bus.$emit('manage--curriculum.update')
        this.loading = false
        this.show = false
      }).catch(e => {
        console.error(e)
        let msg = this.mode == 'add'
          ? 'Unable to add curriculum.'
          : 'Unable to update curriculum.'
        this.$bus.$emit('snackbar--show', msg)
        this.loading = false
      })
    },

    clear() {

      // reset md first
      this.content.forEach((e, i) => {
        let desc = this.$refs['mdDescription-' + i]
        if (desc && typeof desc.reset === 'function') {
          this.$refs['mdDescription-' + i].reset()
        }
      })

      if (this.$refs.form) {
        this.$refs.form.reset()
        if (typeof this.$refs.form.$el.reset === 'function') {
          this.$refs.form.$el.reset()
        }
      }

      this.id = null
      this.label = null
      this.latest = false
      this.status = null
      this.content = []

      if (this.mode == 'add') {
        this.item = null
      } else if (this.mode == 'edit') {
        // set item values if edit mode
        setTimeout(() => {
          this.setValuesFromItem(this.item)
        }, 0)
      }
    }
  }
}
</script>
